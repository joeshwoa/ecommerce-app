...
class BuyerSellerSearch extends StatefulWidget {
  const BuyerSellerSearch({super.key,required this.cubit});
  final BuyerLayoutCubit cubit;

  @override
  State<BuyerSellerSearch> createState() => _BuyerSellerSearchState();
}

class _BuyerSellerSearchState extends State<BuyerSellerSearch> {
  ...
  Future<void> fetchData() async {
    ...
    if (!isLoading) {
      ...
      final responseMap = await post(ApiPaths.sellerSearch+page.toString(),data).onError((error, stackTrace) {
        return {
          'code':999
        };
      });
      ...
      child: Scaffold(
        ...
        body: CustomScrollView(
          ...
          slivers: <Widget>[
            ...
            SliverGrid.builder(
              gridDelegate:
              SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: !isLoading && sellers.isEmpty?1:2),
              ...
              itemBuilder: (context, index) {
                if(!isLoading && sellers.isEmpty) {
                  ...
                  child: Column(
                    ...
                    children: [
                      AppText('قم بكتابة اسم التاجر',color: AppColors.textGreenColor,size: MAsizes.textBigSize),
