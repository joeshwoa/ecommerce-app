...
class BuyerProductPageForSpecificSubCategory extends StatefulWidget {
  const BuyerProductPageForSpecificSubCategory({super.key,required this.categoryID,required this.categoryName,required this.cubit});
  final String categoryID;
  final String categoryName;
  final BuyerLayoutCubit cubit;

  @override
  State<BuyerProductPageForSpecificSubCategory> createState() => _BuyerProductPageForSpecificSubCategoryState();
}

class _BuyerProductPageForSpecificSubCategoryState extends State<BuyerProductPageForSpecificSubCategory> {
  ...
  Future<void> fetchData() async {
    if (!isLoading) {
      ...
      final responseMap = await get("${ApiPaths.getProductBySubCategory}${widget.categoryID}").onError((error, stackTrace) {
        return {
          'code':999
        };
      });
