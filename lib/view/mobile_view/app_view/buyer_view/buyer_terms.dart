...
class BuyerTerms extends StatefulWidget {
  const BuyerTerms({super.key});

  @override
  State<BuyerTerms> createState() => _BuyerTermsState();
}

class _BuyerTermsState extends State<BuyerTerms> {
  ...
  void initState() {
    ...
    if(!isLoading)
    {
      ...
      get(ApiPaths.getBuyerTerms).then((value){
        Map responseMap=value;
        if((responseMap['code']>=200 && responseMap['code']<300))
        {
          terms = responseMap['body']['terms'];
          setState(() {
            isLoading = false;
          });
        }
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.mainColor,
          centerTitle: true,
          title: AppText('سياسات الاستخدام للعميل',size: MAsizes.textNormalSize, color: AppColors.textWhiteColor),
          leading: IconButton(
            onPressed: (){
