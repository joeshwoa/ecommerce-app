...
class BuyerPolicy extends StatefulWidget {
  const BuyerPolicy({super.key});

  @override
  State<BuyerPolicy> createState() => _BuyerPolicyState();
}

class _BuyerPolicyState extends State<BuyerPolicy> {
  ...
  void initState() {
    ...
    if(!isLoading)
    {
      ...
      get(ApiPaths.getBuyerPolicy).then((value){
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.mainColor,
          centerTitle: true,
          title: AppText('سياسات الخصوصية للعميل',size: MAsizes.textNormalSize, color: AppColors.textWhiteColor),
          leading: IconButton(
            onPressed: (){
