...
import 'package:klnakhadem/api/create_objects_from_response/create_buyer_bill.dart';
import 'package:klnakhadem/app_running_data/sharedpreferences.dart';
import 'package:klnakhadem/colors_and_sizes/app_colors.dart';
import 'package:klnakhadem/colors_and_sizes/mobile_app_sizes.dart';
import 'package:klnakhadem/componants/mobile_componants/mobile_bills_for_buyer_card.dart';
import 'package:klnakhadem/componants/app_text.dart';
import 'package:klnakhadem/model/bill_for_buyer_model.dart';

class BuyerMyBills extends StatefulWidget {
  const BuyerMyBills({super.key});

  @override
  State<BuyerMyBills> createState() => _BuyerMyBillsState();
}

class _BuyerMyBillsState extends State<BuyerMyBills> {

  List<BillForBuyer> bills = [];
  ...
  Future<void> fetchData() async {
    if (!isLoading) {
      ...
      final responseMap = await get("${ApiPaths.getBuyerBills}${sharedPreferences.getString('token')}").onError((error, stackTrace) {
        return {
          'code':999
        };
      });
      ...
      if (responseMap['code'] >= 200 && responseMap['code'] < 300) {
        ...
        final List<BillForBuyer> newBills = [];

        for(int i=0;i<responseData["data"].length;i++){
          newBills.add(createBillForBuyer(responseData["data"][i]));
        }
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context,index)=>MAbillForBuyerCard(
                        bill: bills[index],
                      ),
