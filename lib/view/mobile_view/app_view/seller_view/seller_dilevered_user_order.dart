...
import 'package:klnakhadem/componants/mobile_componants/mobile_seller_dilevered_user_order_card.dart';
import 'package:klnakhadem/model/order_model.dart';

class SellerDeliveredUserOrder extends StatefulWidget {
  const SellerDeliveredUserOrder({super.key});

  @override
  State<SellerDeliveredUserOrder> createState() => _SellerDeliveredUserOrderState();
}

class _SellerDeliveredUserOrderState extends State<SellerDeliveredUserOrder> {
  ...
  Future<void> fetchData() async {
    if (!isLoading) {
      ...
      final responseMap = await get("${ApiPaths.getSellerDeliveredUserOrders}${sharedPreferences.getString('token')}").onError((error, stackTrace) {
        return {
          'code':999
        };
      });
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.mainColor,
          centerTitle: true,
          title: AppText('الطلبات قيد التوصيل',size: MAsizes.textNormalSize, color: AppColors.textWhiteColor),
          leading: IconButton(
            onPressed: (){
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: isLoading?const Center(child: CircularProgressIndicator(color: AppColors.mainColor,)):ListView.builder(
                      ...
                      itemBuilder: (context,index)=> MAsellerDileveredUserOrderCard(
