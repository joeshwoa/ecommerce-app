...
class SellerPhoneLogin extends StatefulWidget {
  const SellerPhoneLogin({super.key});

  @override
  State<SellerPhoneLogin> createState() => _SellerPhoneLoginState();
}

class _SellerPhoneLoginState extends State<SellerPhoneLogin> {
                          ...
                          MaterialButton(
                            onPressed: () async {
                              if(formKey.currentState!.validate())
                              {
                                ...
                                Map responseMap = await post(ApiPaths.sellerLogInByPhone,data).onError((error, stackTrace) {
                                  return {
                                    'code':999
                                  };
                                });
                                if((responseMap['code']>=200 && responseMap['code']<300)){
                                  ...
                                  if(active) {
                                    ...
                                    final responseMap2 = await get("${ApiPaths.requestNewSellerOTP}${responseMap['body']['token']}").onError((error, stackTrace) {
                                      return {
                                        'code':999
                                      };
                                    });
                                  ...
                                  if(responseMap['body']['message']=='Incorrect phone or password' || responseMap['body']['message']=='Incorrect phone  or password') {
                                    if (context.mounted){
                                      ScaffoldMessenger.of(context).clearSnackBars();
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                          content: Center(child: AppText('رقم الجوال او كلمة المرور خطأ',color: AppColors.textWhiteColor,size: MAsizes.textNormalSize,)),
                                          backgroundColor: AppColors.mainColor,
                                          showCloseIcon: true,
                                        ),
                                      );
