...
import 'package:klnakhadem/componants/mobile_componants/mobile_seller_dilevered_seller_order_card.dart';
import 'package:klnakhadem/model/order_model.dart';

class SellerDeliveredSellerOrder extends StatefulWidget {
  const SellerDeliveredSellerOrder({super.key});

  @override
  State<SellerDeliveredSellerOrder> createState() => _SellerDeliveredSellerOrderState();
}

class _SellerDeliveredSellerOrderState extends State<SellerDeliveredSellerOrder> {
  ...
  Future<void> fetchData() async {
    if (!isLoading) {
      ...
      final responseMap = await get("${ApiPaths.getSellerDeliveredSellerOrders}${sharedPreferences.getString('token')}").onError((error, stackTrace) {
        return {
          'code':999
        };
      });
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.mainColor,
          centerTitle: true,
          title: AppText('الطلبات الجديدة',size: MAsizes.textNormalSize, color: AppColors.textWhiteColor),
          leading: IconButton(
            onPressed: (){
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: isLoading?const Center(child: CircularProgressIndicator(color: AppColors.mainColor,)):ListView.builder(
                      ...
                      itemBuilder: (context,index)=> MAsellerDileveredSellerOrderCard(
