...
import 'package:klnakhadem/componants/mobile_componants/mobile_seller_return_product_card.dart';
import 'package:klnakhadem/model/return_order_model.dart';

class SellerReturnProductOrder extends StatefulWidget {
  const SellerReturnProductOrder({super.key});

  @override
  State<SellerReturnProductOrder> createState() => _SellerReturnProductOrderState();
}

class _SellerReturnProductOrderState extends State<SellerReturnProductOrder> {
  ...
  Future<void> fetchData() async {
    if (!isLoading) {
      ...
      final responseMap = await get("${ApiPaths.getSellerReturnProductOrders}${sharedPreferences.getString('token')}").onError((error, stackTrace) {
        return {
          'code':999
        };
      });
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.mainColor,
          centerTitle: true,
          title: AppText('طلبات المرتجعات',size: MAsizes.textNormalSize, color: AppColors.textWhiteColor),
          leading: IconButton(
            onPressed: (){
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: isLoading?const Center(child: CircularProgressIndicator(color: AppColors.mainColor,)):ListView.builder(
                      ...
                      itemBuilder: (context,index)=> MAsellerReturnProductCard(
                        price: returnOrders[index].price*(-1),
                        orderNumber: index+1,
